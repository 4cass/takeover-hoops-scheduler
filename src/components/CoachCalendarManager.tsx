...
class CalendarErrorBoundary extends Component<{ children: React.ReactNode }, { hasError: boolean; error: string | null }> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.error("Error caught by boundary in CoachCalendarManager:", error, errorInfo);
  }
...
export function CoachCalendarManager() {
  ...
  const { data: coachId, error: coachIdError } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        ...
        toast.error(`Failed to fetch coach ID: ${error.message}`);
        throw error;
      }
  ...
  const { data: sessions, isLoading, error: sessionsError } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (coachSessionsRes.error) {
        ...
        toast.error(`Failed to fetch coach sessions: ${coachSessionsRes.error.message}`);
  ...
  if (sessionsError || coachIdError || branchesError || packagesError) {
    return (
      <div className="min-h-screen bg-background p-3 sm:p-4 md:p-6">
        <div className="max-w-7xl mx-auto text-center py-12 sm:py-16">
          <CalendarIcon className="w-12 sm:w-14 md:w-16 h-12 sm:h-14 md:h-16 text-gray-400 mx-auto mb-4" />
          ...
          <p className="text-sm sm:text-base md:text-lg text-gray-600">
            Failed to load data: {(sessionsError || coachIdError || branchesError || packagesError as Error)?.message || 'Unknown error'}. Please try again later.
          </p>
        </div>
      </div>
    );
          ...
          <Card className="border-2 border-[#181A18] bg-white shadow-xl">
            <CardHeader className="border-b border-[#181A18] bg-[#181A18] p-4 sm:p-6">
              <CardTitle className="text-lg sm:text-xl lg:text-2xl font-bold text-[#efeff1] flex items-center">
                <CalendarIcon className="h-5 w-5 sm:h-6 sm:w-6 mr-2 sm:mr-3 text-accent flex-shrink-0" style={{ color: '#BEA877' }} />
                  ...
                  {daysInMonth.map(day => {
                    ...
                    return (
                      <button
                        ...
                        `}
                        style={{ borderColor: isSelected || isToday ? '#BEA877' : undefined, backgroundColor: isSelected || isToday ? '#BEA877' : undefined }}
                      >
              ...
              <div className="flex flex-col w-full">
                <DialogHeader className="space-y-2 pb-4">
                  <DialogTitle className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 flex items-center">
                    <Eye className="h-4 w-4 sm:h-5 sm:w-5 mr-2 sm:mr-3 text-accent flex-shrink-0" style={{ color: '#BEA877' }} />
                        ...
                        <CardContent className="p-2 sm:p-3 lg:p-4">
                          <div className="space-y-3 sm:space-y-0 sm:grid sm:grid-cols-2 lg:grid-cols-[repeat(5,1fr)_auto] gap-2 sm:gap-3 lg:gap-4 sm:items-center">
                            <div className="flex items-center space-x-2 min-w-0">
                              <Clock className="h-3 w-3 sm:h-4 sm:w-4 text-accent flex-shrink-0" style={{ color: '#BEA877' }} />
